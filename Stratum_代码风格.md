...
var (
    ...
    ErrVersionNotReady       = errors.New("version not ready")  // PENDING/BUILDING/FAILED 状态不可查询
...
var grpcCodeMap = map[error]codes.Code{
    ...
    ErrVersionNotReady:       codes.FailedPrecondition,
...
- 内部模块（doc store、chunk store、chunk-doc 映射、版本文档列表、布隆过滤器、索引管理器、切割模块）全部放在 `internal/` 下，不对外暴露
...
- `VersionChunkList`：版本 chunk 列表，解决索引构建 N+1 问题
- `BloomFilter`：布隆过滤器
- `IndexManager`：索引管理器
- `RaftNode`：Raft 操作
...
│   ├── chunkdoc/
│   ├── versiondoc/
│   ├── versionchunk/     # VersionChunkList，解决索引构建 N+1 问题
│   ├── bloom/
...
index_manager:
  ...
  load_wait_timeout_ms: 5000    # 并发加载等待超时，超时返回 DEADLINE_EXCEEDED；建议设置为调用方 gRPC 超时的 70%~80%，确保 Stratum 内部超时先于 gRPC 超时触发
  build_trigger_chunk_count: 1000   # 触发异步构建的新增 chunk 数量阈值
  build_trigger_interval_s: 300     # 触发异步构建的最大间隔
...
- 查询成功 / 失败次数
- 索引构建触发次数
- 索引构建成功 / 失败次数（失败时对应 `IndexStatusFailed`）
- 版本回滚次数
- 布隆过滤器假阳性命中次数
- WAL 重放失败次数
