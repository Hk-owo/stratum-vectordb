...
# 保存 → 再分发）在 CI 里根本跑不到。
#
# append_max_dead_ratio 抬高是**发现**，不是顺手：§8.6(c) 只在死向量占比低于该比例时
# 复用父产物，而 §8.6(d) 的收集阈值默认也是 0.2，两者相等就互相抵消——(d) 想收的
# 东西，早在构建时就被 (c) 重建掉了，墓碑留不到被收集的那一刻。实测：删掉 600 篇
# 里的 480 篇，产物是 362 行（整份重建、零墓碑），而不是 (d) 需要的带墓碑的 ~1800 行。
index_manager:
  gc_enabled: true
