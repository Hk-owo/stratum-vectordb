...
```
DeleteVersion
  → RaftNode.ProposeMarkVersionDeleting(kbID, versionID, mode)
      → 状态机 apply 阶段按 mode 确定性地计算待删集合
      → 校验：无活跃版本（ErrVersionIsActive）、无 PENDING（ErrVersionPending）
      → SINGLE：把目标版本的直接子版本改挂到其父版本（splice）
      → ANCESTORS：把目标版本的 parent_version_id 置 0，成为新基底
      → 标记待删集合为 Deleting，返回被删版本 ID 列表
  → [异步] DeleteVersionCoordinator.Execute(kbID)
      → 对每个 Deleting 版本：WAL 标记 → IndexManager.Discard
        → VersionDocList.DeleteByVersion
        → DocStore.DeleteByVersionExceptVisibleFrom（按可见性回收 MVCC 记录）
        → VersionBloomStore.DeleteByVersion（删该版本布隆过滤器与缓存条目）
        → RaftNode.ProposeRemoveVersionMeta → WAL 完成标记（各步均幂等）
  → 返回 success + deleted_version_ids
```
...
- `DeleteVersion → RaftNode.ProposeMarkVersionDeleting`
- `[异步] DeleteVersionCoordinator → RaftNode.ProposeRemoveVersionMeta`
...
type RaftNode interface {
    ...
    ProposeMarkVersionDeleting(ctx context.Context, kbID string, versionID int64, mode VersionDeleteMode) ([]int64, error)
    ...
    ListKnowledgeBases(ctx context.Context) ([]KnowledgeBaseMeta, error)
...
func (r *RaftNodeImpl) ProposeMarkVersionDeleting(ctx context.Context, kbID string, versionID int64, mode VersionDeleteMode) ([]int64, error)
func (r *RaftNodeImpl) ProposeRemoveVersionMeta(ctx context.Context, kbID string, versionID int64) error
...
func (r *RaftNodeImpl) ListKnowledgeBases(ctx context.Context) ([]KnowledgeBaseMeta, error)
...
**版本删除模式（ProposeMarkVersionDeleting）**：由 `mode`（`VersionDeleteMode`）决定待删集合，全部在 apply 阶段基于状态机快照确定性计算，因此各节点结果一致：

- `VersionDeleteSubtree`：目标版本 + 全部后代（默认，历史行为）。
- `VersionDeleteSingle`：仅目标版本；其直接子版本改挂到目标的父版本（splice），分支结构保留。
- `VersionDeleteAncestors`：目标版本的全部前置版本及其旁支；目标的 `parent_version_id` 置 0，成为新基底。

整个待删集合先校验后标记（活跃版本 → `ErrVersionIsActive`、PENDING → `ErrVersionPending`），任一违规则整体拒绝。返回被标记删除的版本 ID 列表。幂等：对已 `Deleting` 的集合重复调用是空操作；`Ancestors` 作用于已是基底的版本返回空列表。清理由 `DeleteVersionCoordinator` 异步完成，见 `[异步] DeleteVersionCoordinator → RaftNode.ProposeRemoveVersionMeta`。
...
**ReplayCounter**：内存态，进程重启归零；重放失败的记录经计数暴露给
`GetSystemStatus`。任一记录重放失败（依赖宕机 / KB 尚未随 raft 追平 / KB 已删）
**不中止启动**：进程内有限退避重试（3 次 × 100ms 指数）后跳过记录、继续启动，
记录保留在 WAL、下次重启重试，数据由 Raft 日志重放 + DataSync 收敛。
...
```go
func (s *Service) DeleteVersion(ctx context.Context, req *DeleteVersionRequest) (*DeleteVersionResponse, error)
```

**入参**：`knowledge_base_id`、`version_id`、`mode`（`VERSION_DELETE_MODE_SUBTREE` / `_SINGLE` / `_ANCESTORS`；不传时为 `SUBTREE`，与历史行为一致）

**未知 mode**：返回 `INVALID_ARGUMENT`，不会静默降级为 `SUBTREE`——否则一个笔误就会删掉整棵子树。

**删除范围**：

| mode | 删除的版本集合 |
|---|---|
| `VERSION_DELETE_MODE_SUBTREE`（默认） | `version_id` 及其全部后代版本 |
| `VERSION_DELETE_MODE_SINGLE` | 仅 `version_id`；其直接子版本改挂到 `version_id` 的父版本上（链表 splice），分支结构保留——用于单独删除任意「中间版本」 |
| `VERSION_DELETE_MODE_ANCESTORS` | `version_id` 的全部前置版本，以及这些祖先上挂着的其它分支；`version_id` 的 `parent_version_id` 置 0，成为版本链新的基底 |

**约束**（在 Raft 状态机 apply 阶段确定性校验，节点间一致）：版本必须存在且属于该知识库；待删集合中不得包含当前活跃版本（`ErrVersionIsActive`），也不得包含任何 PENDING 版本（`ErrVersionPending`）。约束对**整个待删集合**生效：任一版本触发约束则整体拒绝，不会出现半删除状态。

另有一条**幸存版本约束**：若有版本存活下来、但其父版本位于待删集合中（`SINGLE` 被重挂的子版本、`ANCESTORS` 中被保留的目标版本），该幸存版本不得处于 PENDING——它完成存储写入（`WriteCoordinatorImpl.writeVersionDocList` 读父版本的 `VersionDocList`）与崩溃恢复（按 `ParentVersionID` 重放）都依赖那个即将被清理的父版本，否则会静默得到不完整的文档集；命中时返回 `ErrVersionPending`。

**存储回收语义**：MVCC 文档记录是**增量写入**的（每个版本只写自己改动的文档），读取时按版本回溯到 ≤ 目标版本的最新记录。因此清理被删版本时，那些**仍被存活版本读取**的记录会被保留（`DocStore.DeleteByVersionExceptVisibleFrom`），否则存活版本会静默丢失该文档或读到更旧的值。判定锚点是最小的存活更高版本：记录若仍是该版本的读取来源则保留。只有当一个版本之上不存在任何存活版本时，其记录才被完全回收。

因此 `SINGLE` / `ANCESTORS` 删除中间或前置版本**不会立即释放**这些文档占用的磁盘空间——它们仍是存活版本的活跃数据；完全回收发生在删除链尾（无存活更高版本）时。

**已知限制**：为存活版本保留下来的记录，在该存活版本随后也被删除时不会被立即回收（一次清理只处理被删版本自身的记录），会留下不再被任何版本读取的孤儿 MVCC 记录。它们不影响查询——每个版本的可检索文档集由该版本自己的索引与 `VersionDocList` 决定——但会占用磁盘。**该回收目前尚未实现**：除整库删除（`DeleteByKB` 清理该库全部记录）外没有自动收敛路径，需要后续补一个存储层 GC。

被删版本自身的索引（内存条目、vecstore 侧索引、磁盘 `.index`/`.ids`/`.mem`）与每版本文档布隆过滤器（`bloom-version/<kb>/<version>.bloom`）均**随版本删除一并回收**；chunk 向量（L2）仍由 `ChunkGarbageCollector` 按"最新版本是否引用"周期性回收，不随版本立即删除。

**返回**：`success`、`deleted_version_ids`（本次实际标记删除的版本 ID 列表；`ANCESTORS` 会包含被波及的旁支版本）。标记完成后清理异步执行；重复调用幂等，`ANCESTORS` 作用于已是基底的版本是空操作（返回空列表）。
...
```go
func (s *Service) ListKnowledgeBases(ctx context.Context, req *ListKnowledgeBasesRequest) (*ListKnowledgeBasesResponse, error)
```

**入参**：无

**返回**：知识库列表，每条记录包含 `knowledge_base_id`、`name`、`chunk_window_size`、`chunk_overlap_size`、`index_type`、`similarity`、`embed_config`、`active_version_id`、`status`（ACTIVE / DELETING / DELETE_FAILED）
...
```go
func (s *Service) GetKnowledgeBase(ctx context.Context, req *GetKnowledgeBaseRequest) (*GetKnowledgeBaseResponse, error)
```

**入参**：`knowledge_base_id`

**返回**：单个知识库元数据，字段同 ListKnowledgeBases 的单条记录
...
- `resource_usage`：资源使用快照（`loaded_index_count` 已加载索引数、`chunk_store_bytes` vecstore RocksDB 大小、`doc_store_bytes` DocStore PebbleDB 大小）
