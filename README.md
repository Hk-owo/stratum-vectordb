...
|---|---|---|
| 冷查询(重启副本后的首次,含磁盘 `Load`) | **3.80 ms** | **6.88 ms** |
| p50(热) | **2.93 ms** | **5.10 ms** |
| p95 | **4.56 ms** | 7.39 ms |
| p99 | **8.94 ms** | 9.69 ms |

**同一台机器、同一口径下的四轮对比**(2,000 篇,2026-09;见下"查询路径的三处修复"):
...
单次查询的成本合计降了 **约 230×**。**规模效应也回到了合理区间**:4× 数据(2,000 → 8,000 篇)只带来 **1.74×** 延迟(2.93 → 5.10 ms)——修复前同口径是 12–13×,而且是秒级。**cold − warm ≈ 0.9 ms** 才是从磁盘 `Load` 产物本身的价格。
...
**查询向量的口径**:套件**现在发确定性非零向量**(`queryVector(768)`,固定种子),与真实调用方一致。曾经每个查询都发全零向量 —— 它与所有文档等距,HNSW 贪心遍历无从剪枝、退化为近似全图扫描;那是真实存在的放大效应,但只是**放大器**:延迟基数掉到 ms 级之后,保留的零向量对照是 **4.70 ms vs 2.93 ms**(2,000 篇,+60%)与 **10.02 ms vs 5.10 ms**(8,000 篇,约 2×),而不是数量级。
...
| 指标 | 值 |
|---|---|
| 写入(`CreateVersion` × 10 批,仅 API 用时) | **318 ms** |
| 累计索引构建(10 批串行,每批等前一批 READY) | **4 m 7 s** |
| 端到端 | **4 m 7 s** |
| 存储占用(存储节点 1 / 2 / 3) | **490.2 / 211.6 / 197.3 MiB**,合计 **899 MiB** |
| 查询 | top-k=10 返回 10 条结果 |

读法:**写入 API 从 2.55 s 降到 318 ms(8×)**,与控制面连接复用是同一处收益(写入路径的 `proposeAt` 也复用连接了);而 4 分钟仍然全在索引构建上 —— "每版本独立索引"模型下吞吐由构建决定,这一项不受连接复用影响。存储占用是**节点数据目录总量**(含卷里累积的历史知识库),跨轮次不可直接比较。
...
| 用例 | 测什么 | 实测 |
|---|---|---|
| `TestT4_QueryLatency` | 单版本查询延迟,**冷热分开报** | 2,000 篇 / 200 次查询:cold **3.80 ms** · p50 **2.93 ms** · p95 **4.56 ms** · p99 **8.94 ms**;8,000 篇:cold **6.88 ms** · p50 **5.10 ms**(确定性非零查询向量;同口径最初为 cold 769 ms / p50 679 ms) |
