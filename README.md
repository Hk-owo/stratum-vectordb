...
| 指标 | 值 |
|---|---|
| 冷查询(重启副本后的首次,含磁盘 `Load`) | **3.7 ms** |
| p50(热) | **3.0 ms** |
| p95 | **4.6 ms** |
| p99 | **8.3 ms** |

**同一台机器、同一口径下的四轮对比**(2026-09,见下"查询路径的三处修复"):
...
| **再把控制面读的连接复用起来** | **3.7 ms** | **3.0 ms** | 4.6 ms | 8.3 ms | 45 s |

单次查询的成本合计降了 **约 220×**。**cold − warm ≈ 0.7 ms** 才是从磁盘 `Load` 产物本身的价格;3 ms 里剩下的部分是本地几段微秒级工作加一跳服务站的网络往返。
...
**查询向量的口径**:套件**现在发确定性非零向量**(`queryVector(768)`,固定种子),与真实调用方一致。曾经每个查询都发全零向量 —— 它与所有文档等距,HNSW 贪心遍历无从剪枝、退化为近似全图扫描;那是真实存在的放大效应,但只是**放大器**:延迟基数掉到 ms 级之后,保留的零向量对照是 **4.4 ms vs 3.0 ms**(+45%),而不是数量级。
...
| 用例 | 测什么 | 实测 |
|---|---|---|
| `TestT4_QueryLatency` | 单版本查询延迟,**冷热分开报** | 2,000 篇 / 200 次查询:cold **3.7 ms** · p50 **3.0 ms** · p95 **4.6 ms** · p99 **8.3 ms**(确定性非零查询向量;同口径最初为 cold 769 ms / p50 679 ms) |
