...
```
  ...
  │  ┌────────────────────────────────────────────┐ │
  │  │ 内存 L0(每版本一份,LRU 管理)                 │ │
  │  │   · 全精度路径(默认):IndexHNSWFlat           │ │
  │  │   · 量化路径:粗筛器 = HNSW 图 + SQ/PQ 量化码   │ │
  │  │   · 生命周期状态机 EMPTY/BUILDING/READY +     │ │
  │  │     每实例读写锁(读/写按状态分门,防竞态)        │ │
  │  └──────────────────┬─────────────────────────┘ │
  │                     │ ① 粗筛 → 候选 top-N         │
  │                     ▼                            │
  │  两段式 Search ② ReadMulti 按候选读全精度原向量     │
  │  (量化 KB)   ③ 全精度精确 rerank → 最终 top-k     │
  │                     │                            │
  │  ┌──────────────────▼─────────────────────────┐ │
  │  │ 磁盘 L1/L2                                 │ │
  │  │   · RocksDB chunk store = 全精度向量权威(L2) │ │
  │  │   · <ver>.index(含码本)/.ids/.mem 冷层(L1)   │ │
  │  └────────────────────────────────────────────┘ │
  └─────────────────────────────────────────────────┘
```

> 两条检索路径、量化类型与并发模型详见「向量检索:全精度与量化」。
...
每个知识库在创建时固定索引与量化配置(创建后不可变,变更需新建 KB 迁移)。默认 `OFF`(全精度),行为与"单段精确检索"一致;可选量化把**内存驻留物从"全精度向量"换成"量化粗筛器"**。
...
| 查询 | 单段:内存直接出 top-k(精确) | 两段式:① 粗筛 top-N 候选 → ② 按候选从 RocksDB 读全精度原向量 → ③ 精确 rerank 出 top-k |
| 磁盘 | `.index` 文件;向量在 RocksDB | 同左(全精度向量**不压缩**、永驻 RocksDB) |
| 最终精度 | 精确 | **由 rerank 的全精度向量决定**;量化只影响候选覆盖(召回) |

最终评分/阈值口径在两条路径上一致(统一"分高者相似";COSINE 归一化后内积)。

### 量化类型(每向量载荷;Faiss 内建,码本随 `.index` 持久化,Load 免配置)

| 类型 | 载荷 | 训练 | 单版本载荷内存(d=768,n=100 万,示例) | 备注 |
|---|---|---|---|---|
| `OFF`(Flat) | 4B/维 | 无 | ≈ 3.07 GB | 默认;单段精确 |
| `SQ8` | 1B/维 | 首包一次(逐维范围) | ≈ 768 MB | 合成数据召回≈Flat |
| `SQ_FP16` | 2B/维 | 免训练 | ≈ 1.54 GB | 同 SQ8,推荐档 |
| `SQ_BF16` | 2B/维 | 免训练 | ≈ 1.54 GB | **无硬件 bf16 时走软件慢路径,延迟 ~7×,建议回避** |
| `PQ`(m,8bit) | `m` B/向量 | 首包 k-means(码本) | ≈ 96 MB(m=96) | 最省;召回随候选 N 提升,建议加大候选 |

> 量化后 **HNSW 图边成为内存主导项**(实测 d=64、M=32 ≈ 272 B/节点)——分级记账含图边与量化码。

### 生命周期与并发(量化器"写"与"读"分门)

每个 `(kb, version)` 索引对象 = 一个量化粗筛器,带显式状态机与每实例锁:

- 状态:`EMPTY`(空/Reset,Search 返回空集)→ `BUILDING`(首个 Build 起,只写不读,Search 返回明确错误)→ `READY`(Save 密封或 Load 恢复,可查);
- 锁:Search(含 rerank 磁盘 IO)持**读锁贯穿**;Build/AddChunks/Save/Load/Reset 持**写锁**并完成状态转换;service 锁只管索引表;
- 结果:Search ∥ Reset/Load/AddChunks 在对象级互斥,杜绝 use-after-free/错位结果(设计文档 v12 §2.6)。

### 分级存储(内存只装粗筛器)

- **L0 内存热层**:每版本粗筛器,IndexManager 以 LRU 管理;字节记账用 vecstore 报告的粗筛器口径(`Build/AddChunks` 响应 `mem_bytes`,图边+码),OFF 仍按 `4×d×n`;
- **L1 磁盘冷层**:每版本 `<versionID>.index`(+ `.ids`/`.mem` 边车)——保留策略(`gc.version_retention_count`)裁剪;Load 即恢复(文件自描述);
- **L2 永久层**:RocksDB 全精度向量(内容寻址、不压缩)。

换出只丢小粗筛器(重载便宜),**不影响查询结果精度**(rerank 永远读 L2 全精度)。

### 实测(2026-09,合成数据;方法/局限见设计文档 v12 §2.5 与附录 D)

- 召回:SQ8/SQ_BF16/SQ_FP16 ≈ Flat HNSW;PQ 较低且随候选 N 提升(0.56@32 → 0.75@256);
- 端到端查询(热 cache、单线程,top10、候选 N=80):OFF ≈ 0.27 ms,SQ8/FP16 ≈ 0.71 ms(2.6×),PQ ≈ 1.0 ms(3.7×)——增量来自候选放大读盘 + rerank;按需调小 `candidate_n` 或保持 OFF。
