...
|---|---|---|---|---|---|---|
| 冷查询 | 17.03 ms | 6.60 ms | 22.07 ms | 18.34 ms | 25.15 ms | 277.72 ms |
...
**量化买到的是内存,不是延迟**:`SQ8` 把每向量的内存载荷从 4 B/维压到 1 B/维,而这条路径的延迟**不降**——两段式的粗筛省下的时间,被"按候选回读全精度向量做 rerank"吃掉了。本轮 2,000 篇上 `SQ8` 比全精度高 0.52 ms p50,8,000 / 20,000 篇上反而低 1.81 / 1.87 ms(20,000 篇的冷查询 277.72 ms 是一次异常样本)。**三个方向都落在轮间噪声里,能确定的只有"没有收益"**:量化的收益在内存与产物字节(见下),不在查询延迟。要不要开量化,取决于部署的内存压力(以及 §2.4 里那张载荷表)。写入路径同理:量化只影响索引构建(见下),不影响写入事务。
