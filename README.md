...
| `DeleteVersionData` | 回收本节点上某版本的物理数据(**删除版本**时向所有候选副本广播:控制面不知道字节在哪几个节点上;判死后不再走这条广播,改为每个 Raft 成员从自己的 apply 得知后本地回收(不参与 Raft 的存储节点只有"检测者就地清"与"重启扫一次"两条);**接收端无法执行清理时以错误回答**,不静默成功——否则控制面会先删掉元数据行,这里的残骸再也没人能命名) |
...
全量单测(25 个测试包)与 3 节点 Docker 集群(T4)命令见[快速开始](#快速开始)。

CI(`.github/workflows/ci.yml`,push main 与 PR):gofmt + `go vet` + `go build` + 全量单测(`go test ./...`,25 个测试包) + raft/kvraft/index 竞态检测 + 3 节点 Docker 集群(T4)容错运行;C++ vecstore 由手动触发的工作流(`vecstore-cpp.yml`)覆盖。
...
> 以下为 **2026-09-21** 在两层拓扑(控制组 3 + 存储组 3,每个存储容器自带真实 Faiss HNSW + RocksDB,768 维)上的实测,由 `scripts/cluster.sh --topology two-tier` 起集群、经服务站测量。宿主 **12 核 / 15 GB**,容器与压测进程共享这台机器。测量前集群镜像按当前代码重建、数据卷清空重来(数据目录 1.4–2.2 GB,不是此前轮次累积的 8–19 GB)。**完整口径、原始数据与本轮发现(两条产品缺陷 + 一条测试工具缺陷,均已修)见 `docs/stress-test-report.md`**,本节只是它的摘要。
...
|---|---|---|---|
| 冷查询(重启副本后的首次,口径见下) | **12.00 ms** | **45.72 ms** | **49.63 ms** |
| p50(热) | **4.85 ms** | **10.81 ms** | **35.50 ms** |
| p95 | **6.61 ms** | **13.29 ms** | **61.17 ms** |
| p99 | **7.17 ms** | **16.55 ms** | **105.56 ms** |
| 平均 | 5.06 ms | 11.30 ms | 38.73 ms |
| 全零向量对照 p50 | 4.41 ms | 10.64 ms | 34.72 ms |

查询向量与 embedder 同卦限(`queryVector(768)`,固定种子、分量非负),所以 HNSW 的剪枝强度与真实调用方一致;全零向量那行是同一次运行内的对照——它与所有文档等距,贪心遍历无从剪枝,是"测了一个没人会发的请求"的代价。热 p50 与文档数**比线性更陡**(4.85 → 10.81 → 35.50;文档数比 1:4:10,延迟比 1:2.23:7.32):本套语料经内容定义分块去重后唯一 chunk 极少,`matched_docs` 恒等于全部文档数,所以这条路径量的是"候选映射到多少个文档",而这一段的工作量随文档数放大后,尾部分位(p95/p99)比 p50 涨得更快——20,000 篇的 p99/p50 已到 2.97(2,000 篇 1.48)。

**量化对照** —— 同一个用例、同一台机器,只把知识库创建时的量化类型换成 `SQ8`(创建后不可变):

| 指标 | 2,000 篇 `OFF` | 2,000 篇 `SQ8` | 8,000 篇 `OFF` | 8,000 篇 `SQ8` | 20,000 篇 `OFF` | 20,000 篇 `SQ8` |
|---|---|---|---|---|---|---|
| 冷查询 | 12.00 ms | 11.17 ms | 45.72 ms | 18.64 ms | 49.63 ms | 29.82 ms |
| p50(热) | 4.85 ms | 5.79 ms | 10.81 ms | 11.59 ms | 35.50 ms | 34.90 ms |
| p95 | 6.61 ms | 9.01 ms | 13.29 ms | 14.99 ms | 61.17 ms | 58.52 ms |
| p99 | 7.17 ms | 12.71 ms | 16.55 ms | 17.75 ms | 105.56 ms | 78.64 ms |
| 平均 | 5.06 ms | 6.24 ms | 11.30 ms | 12.00 ms | 38.73 ms | 37.29 ms |
| 全零向量对照 p50 | 4.41 ms | 5.33 ms | 10.64 ms | 10.84 ms | 34.72 ms | 34.22 ms |

**量化买到的是内存,不是延迟**:`SQ8` 把每向量的内存载荷从 4 B/维压到 1 B/维,而这条路径的延迟**不降**——两段式的粗筛省下的时间,被"按候选回读全精度向量做 rerank"吃掉了。本轮 2,000 / 8,000 篇上 SQ8 比全精度高 0.8–0.9 ms p50,20,000 篇上两者持平(34.90 vs 35.50,−0.6 ms);上一轮同一档是 +4.1 ms。**两个方向都落在轮间噪声里,能确定的只有"没有收益"**:量化的收益在内存与产物字节(见下),不在查询延迟。要不要开量化,取决于部署的内存压力(以及 §2.4 里那张载荷表)。写入路径同理:量化只影响索引构建(见下),不影响写入事务。
...
**全精度(`OFF`)** —— 各 246 / 248 个查询,p50 / p95(µs):

| 段 | 做什么 | 2,000 篇 p50 | 2,000 篇 p95 | 8,000 篇 p50 | 8,000 篇 p95 |
|---|---|---|---|---|---|
| `filter_us` | 候选的版本过滤 + 聚合(`chunkmap_us` 是它的子项) | **3132** | 5539 | **10613** | 17128 |
| ↳ `chunkmap_us` | 每个候选 chunk 一次 `ListDocIDs`(chunk → 文档) | 1974 | 3962 | 5329 | 9694 |
| `bloom_us` | bloom 过滤器 + 该版本的文档 ID 集合(各一次本地读) | 321 | 708 | 1208 | 2395 |
| `meta_us` | 向控制面读版本元数据(`ListVersions`,连接已复用) | 319 | 819 | 396 | 1113 |
| `search_us` | vecstore 的 HNSW 检索 | 315 | 662 | 400 | 1437 |
| `read_us` | 读命中文档的正文 | 173 | 297 | 246 | 434 |
| **`total_us`** | 节点内合计 | **4558** | 8479 | **14247** | 22822 |

**量化(`SQ8`)** —— 同口径:

| 段 | 2,000 篇 p50 | 2,000 篇 p95 | 8,000 篇 p50 | 8,000 篇 p95 |
|---|---|---|---|---|
| `filter_us` | **2853** | 5864 | **10466** | 25039 |
| ↳ `chunkmap_us` | 1495 | 3983 | 5088 | 11594 |
| `bloom_us` | 219 | 558 | 1953 | 4677 |
| `meta_us` | 353 | 1054 | 382 | 3105 |
| `search_us` | 431 | 1510 | 471 | 1869 |
| `read_us` | 85 | 144 | 150 | 344 |
| **`total_us`** | **4558** | 8041 | **15176** | 34802 |

读法:各段 p50 之和看起来大于 `total_us`,因为 `chunkmap_us` 是 `filter_us` 的子项(它在过滤循环内部累加)。真正的大头是**与候选规模成正比**的那两项(`chunkmap_us` + 过滤本身),而不是检索;`search_us` 占比始终是个位数百分比(全精度 2,000 篇 6.9%、8,000 篇 2.8%)——这条路径对 top-k 与候选数敏感,对文档总量只通过"候选映射到多少文档"间接敏感。本套语料是合成的高重复语料(6 句循环拼接),唯一 chunk 极少,所以候选数恒为 11、`matched_docs` 等于全部文档数;量化的两段式在这里也体现不出粗筛的规模优势,真实语料下量化的收益是**内存**(§2.4 的载荷口径),不是这条路径的延迟。本轮与上一轮的同一张表比:走势与构成不变,**绝对值整体高约 1.7–2.5 倍,且集中在 `filter_us`/`chunkmap_us`**;两轮集群的数据目录规模、页缓存状态与运行时长都不同,这张表只用于看构成与占比,不用于跨轮比较(§3.3)。
...
|---|---|---|---|---|
| 写入(`CreateVersion` × 10 / 20 批,仅 API 用时) | **456.53 ms** | **524.67 ms** | **1.246 s** | **907.48 ms** |
| 累计索引构建(10 / 20 批串行,每批等前一批 READY) | **2 m 11.1 s** | **2 m 07.1 s** | **7 m 20.9 s** | **6 m 46.2 s** |
| 端到端 | **135 s** | **131 s** | **446 s** | **412 s** |
| 数据目录(3 个存储节点合计,本轮新建) | 1.37 GB | 1.46 GB | 1.84 GB | 2.15 GB |
| 单版本索引产物(1,000 篇 / 11 chunk,`debug` 轮的 `build timings`) | 33,792 B | **11,440 B** | 同左 | 同左 |
| 查询 | top-k=10 返回 10 条结果 | 同左 | 同左 | 同左 |

四列来自同一次压测、同一集群,差别只在规模与知识库创建时的量化类型(创建后不可变)。**索引产物**是量化在这里唯一干净的可比量:`.index` 同时装 HNSW 图与向量载荷,同一份数据下 SQ8 的产物约为 `OFF` 的 34%(1,000 篇:11,440 / 33,792)。

两件从这张表里读出来、且不能只读数字的事:**写入 API 的用时与规模几乎无关**(10,000 篇 457 ms、20,000 篇 1.25 s,摊到每批 45–62 ms)——版本提交在批量固定时是常数级操作,数据落盘与索引构建都在响应返回之后;**累计构建则超线性**(10 批 2.19 m → 20 批 7.35 m,规模翻倍 3.4×),因为每个版本都要重写完整的 doc-ID 集并重建整个索引,这是"每版本独立索引"模型的固有开销,不是回归。此外**产物字节与文档规模无关**:本套语料高度重复,内容定义分块去重后各规模的唯一 chunk 集一样大,而 `.index` 装的是 chunk(内容寻址)而不是文档——这也正是"产物大小不能用来衡量文档规模"的原因(上一轮同一条产物是 36,966 / 17,802 B:语料与 chunk 数变了,结论不变)。

存储节点数据目录总量此前**不可跨轮比较**(它统计的是整个数据目录,含历次压测留下的知识库,只会随轮次单调变大);本轮数据卷是清空重来的,所以上表那一行(1.37–2.15 GB)是**干净口径**,但正因如此它与此前各轮的数字不可比。
...
实测一例(3+3 容器集群,一批 1,000 篇;前三列是同一写法的三次独立测量,第四列换量化类型):

| 段 | 1,000 篇(DataVolume) | 1,000 篇(QueryLatency 2,000 档) | 1,000 篇(QueryLatency 8,000 档) | 1,000 篇(SQ8) |
|---|---|---|---|---|
| 段 1 `total_us`(其中 `leader_lookup_us` / `attempt_us`) | 64.3 ms(8.4 / 55.9) | 28.9 ms(3.0 / 25.9) | 29.3 ms(3.4 / 25.9) | 64.7 ms(27.9 / 36.8) |
| 段 2 `total_us`(其中 `gate_us` / `convert_us` / `execute_us`) | 8.4 ms(0.02 / 0.03 / 8.3) | 9.6 ms(0.01 / 0.03 / 9.6) | 11.5 ms(0.01 / 0.02 / 11.5) | 5.1 ms(0.02 / 0.02 / 5.1) |
| 段 3 `total_us`(其中 `txn_wait_us` / `propose_us` / `handoff_us`) | 8.3 ms(0 / 8.3 / 0.01) | 8.8 ms(0 / 8.8 / 0.01) | 11.4 ms(0 / 11.4 / 0.01) | 4.5 ms(0 / 4.5 / 0.01) |
| 段 4 `total_us`(其中 `tried_us` 唯一那次) | **4,771.1 ms**(4,771.1) | 10,559.6 ms(10,559.6) | 28,277.6 ms(28,277.6) | 4,947.4 ms(4,947.4) |
| 段 5 `total_us`(其中 `local_us` / `fanout_us` / `report_us`) | **4,683.7 ms**(2,387.8 / 2,289.2 / 6.7) | 10,487.1 ms(2,519.0 / 7,958.7 / 9.4) | 28,203.3 ms(5,719.5 / 22,478.4 / 5.3) | 4,782.5 ms(2,400.6 / 2,369.8 / 12.2) |
| 段 6 `total_us`(其中 `queue_us` / `build_us` / `size_bytes`) | **29.3 ms**(0.08 / 29.2 / 33,792) | 26.9 ms(0.22 / 26.7 / 33,792) | 297.2 ms(0.06 / 297.1 / 33,792) | 140.7 ms(1.07 / 139.6 / 11,440) |

四条从这张表里读出来、别处看不到的事:

- **客户端等的时间主要不是节点算的。** 段 1 的 64.3 ms 里,节点内部只占 8.4 ms(段 2),其余 ~56 ms 是把这 1,000 篇搬过服务站→控制节点那条 wire(收发 + 序列化);同一轮里另两列同一差额只有 ~20 ms,可见它随当时的 wire/负载抖动,而不随请求数走。
- **批量写的成本在复制,不在本地处理。** 段 5 的 4.68 s 里 `local_us` 2.39 s(切分 + embed + 落盘)、`fanout_us` 2.29 s。要压批量写入的墙钟时间,该看的是副本侧,不是本机——第三列就是证据:同一批 1,000 篇,`fanout_us` 从 2.29 s 抖到 **22.48 s**(近 10×),整批墙钟随之涨到 28.2 s。
- **段 4 量的不是"挑候选",是"等候选干完"。** 挑候选只花 `resolve_us`(读副本拓扑 + 排序 + 健康排序,四列都是 **4–5 µs**);`tried_us` 里那 4,771.1 ms 是**候选节点执行写事务**的墙钟时间,它 ⊇ 段 5 的 4,683.7 ms。要问"轮询了几个候选",看的是 `tried` 的长度:长度为 1 且其值 ≈ 段 5 ⇒ 第一个候选就接下来了(本轮四列都是这一种);多个元素、每个都逼近 `budget_ms`(1,000 篇时 65 s)⇒ 前面的候选不可达、各自吃满了预算(§7.13.2 的候选轮询),那才是段 4 真正会爆的时候。
- **"等 READY"和"构建索引"不是一回事。** 测试用例报的 `build-accum`(从 `CreateVersion` 返回到 READY)在 1,000 篇时是 5.06 s(全精度)/ 5.55 s(SQ8),而段 6 的 `build_us` 只有 29.2 / 139.6 ms——两者差 36–173 倍,因为 `CreateVersion` 在**版本提交**时就返回了,之后的 5 s 是段 4 的数据落盘,不是构建。拿 `build-accum` 当"索引构建耗时"会把它高估一到两个数量级。
...
| 段 | 做什么 | 1,000 篇(全精度) | 1,000 篇(SQ8) |
|---|---|---|---|
| `acquire_us` | 该知识库在途写入的限流(§7.7) | 2 µs | 5 µs |
| `local_us` | 本地事务:切分 → embed → 落盘 | **2,387.8 ms** | **2,400.6 ms** |
| `fanout_us` | 复制到副本(quorum;对端各自拉取并落盘) | **2,289.2 ms** | **2,369.8 ms** |
| `report_us` | 上报"数据已可持久" + 触发索引构建 | 6.7 ms | 12.2 ms |
| `confirm_us` | 广播确认(异步触发,基本不计入) | 4 µs | 4 µs |
| **`total_us`** | 节点内合计 | **4,683.7 ms** | **4,782.5 ms** |

本轮两列来自同一轮 `debug` 测量(各自知识库的第一次写入),所以 `local_us` / `fanout_us` 的差异(1%–4%)是环境抖动而不是量化造成的。可比的是**量与构成**:量化的影响只在段 6(见下),不在这条事务里——同规模的四列 `local_us` 都在 2.4–5.7 s,而 `fanout_us` 在 2.3–22.5 s 之间抖,后者才是批量写入墙钟的主变量。`report_us` 很小,却是最不能少的一步:它同时是"数据已可持久"的上报与索引构建的触发点。

同一规模(1,000 篇)下 SQ8 对**索引构建**的影响看段 6:`build_us` 从 **29.2 ms** 升到 **139.6 ms**(训练码本),`size_bytes` 从 **33,792** 降到 **11,440**——这就是量化买到的内存,与写入事务无关。
...
| 用例 | 测什么 | 实测 |
|---|---|---|
| `TestT4_QueryLatency` | 单版本查询延迟,**冷热分开报** | 2,000 篇:cold **12.00 ms** · p50 **4.85 ms** · p95 6.61 ms · p99 7.17 ms;8,000 篇:cold **45.72 ms** · p50 **10.81 ms** · p95 13.29 ms · p99 16.55 ms;20,000 篇:cold **49.63 ms** · p50 **35.50 ms** · p95 61.17 ms · p99 105.56 ms |
| `TestT4_DataVolume` | 写入量:写 / 构建 / 占用 | 10,000 篇(10 批 × 1,000):写入 API **457 ms** · 累计索引构建 **2 m 11 s** · 数据目录 **1.37 GB**;20,000 篇(20 批):写入 **1.25 s** · 构建 **7 m 21 s** · **1.84 GB**;量化(SQ8)同口径:10,000 篇 **525 ms / 2 m 07 s / 1.46 GB**,20,000 篇 **907 ms / 6 m 46 s / 2.15 GB** |
...
| `TestT4_MultiVersionEviction` | 多版本分级换出的稳定性 | 8 版本 × 3 轮轮转、24 次查询,每个版本始终应答(49 s) |
| `TestT4_GCPressure` | 墓碑回收的端到端可见性(写入 → 删除 → 观察产物 → 查询仍正确) | **产物 99,194,875 B → 24,326,922 B(回收 75.5%)**,§8.6(c) 在 **3/3** 副本上复用了父产物,收集后查询仍返回 5 条结果;用时 75 s。这条路以前从来没真正跑通过——它被四层套着的因果挡着(delta 为空导致 §8.6(c) 重建、判据无日志、分发获得的副本不上报 READY、`IndexReadyNodes` 不过 wire),见 `docs/stress-test-report.md` §6。2026-09-20 那轮它又变成 flaky(8 次里 3 次失败):§8.6(d) 补上第二个扫描来源"链尾"后,刚写完的版本 5 s 内就被回收,而当时用"产物比父的干净产物大"推断"复用了、有墓碑",测量晚于收集时读到的是回收后的产物,于是把"墓碑刚被回收"误判成"重建了、没有墓碑"。**已改为用日志证据断言**(`built by appending to the parent version's artifact (§8.6c)` + `deleted_chunks > 0`),对"复用产物"与"父的干净产物"两个参照都成立;本轮连同 `-count=10` 均通过 |

测量类用例的规模都可由环境变量放大,默认值小到能进 CI;放大时记得同时放大 `STRATUM_STRESS_TIMEOUT`(20,000 篇建议 `90m`)。**本轮(2026-09-21,数据卷清空重来 + 代码重建镜像)抓到三处缺陷,三处都已修**(详见 `docs/stress-test-report.md`):① **测试读日志的方式**会漏掉正在找的那一行——容器被 SIGKILL 重启后 docker 在 json 日志里写了一条分隔行,而 docker 29.1.3 的 `docker logs --tail 5000` 会停在那里(实测:文件 26,815 行、末行是当下写的,`docker logs` 只返回 22,634 行、停在 30 分钟前),于是"追赶已完成"被判成"从未追上"⇒ `nodeLogsSince` 改直读容器 json 日志并逐行跳过坏行(§7.4 的同类坑这些年第三次出现);② **vecstore 对"尚未落盘的 chunk 向量"答 `NOT FOUND`**,而 `isTransientBuildErr` 只认 Unavailable/DeadlineExceeded/FailedPrecondition/Internal ⇒ 一次"构建跑在数据同步前面"的竞态就把版本标成 `FAILED_PERMANENT`,**在所有副本上不可查**(包括已经建好索引的那些),只能人工 rebuild;现在这一类归入可重试,交给已有的 5 分钟重试窗口;③ **"文档在、chunk 不在"的副本把"我还不能服务"答成"没有匹配"**(§7.4 修过的两条空路径之外的第二条):文档集读到且非空、chunk 映射没到 ⇒ 本副本索引为空而**搜索成功返回 0 候选**,前两条判据都不会命中,服务站按设计把空结果当答案,真正持有该版本的两台副本根本没被问;现在 `Search` 在返回空前先问一句"这些文档的 chunk 在本副本上有吗"。同上,每条都带"撤掉即变红"的单测。
...
# 分段计时(上面的"查询成本的构成"与"写入成本的构成"):集群与服务站都换 debug 级,
# 并让套件复用**同一个**服务站进程(STRATUM_T4_STATION_ADDR)
LOG_LEVEL=debug scripts/cluster.sh --topology two-tier up --force
go build -o /tmp/stratum-router ./cmd/stratum-router/
setsid nohup /tmp/stratum-router -listen 127.0.0.1:7020 \
  -nodes localhost:17000,localhost:17001,localhost:17002 \
  -storage-nodes localhost:17100,localhost:17101,localhost:17102 \
  -log-level debug > /tmp/station-debug.log 2>&1 &
export STRATUM_T4_STATION_ADDR=127.0.0.1:7020
STRATUM_STRESS_DOCS=2000 STRATUM_STRESS_QUERIES=224 go test ./integration/docker/ -tags=docker -count=1 -run TestT4_QueryLatency -v
STRATUM_VOLUME_DOCS=1000 go test ./integration/docker/ -tags=docker -count=1 -run TestT4_DataVolume -v
```
