...
**`TestT4_GCPressure` 目前会 SKIP**,原因是真实的:它能造出墓碑(删掉 600 篇中的 480 篇后,索引字节 +184 KB,即增长 138%),但扫描器没有把它判为候选。已定位到两个可疑点并记入待办(设计文档 §5):夹具数据的 `chunk` 数量远低于文档规模(600 篇只落出约 18 个 chunk id),以及**卷里残留旧数据时,reconcile 的重建风暴会饿死新版本的构建**。它跳过而不是失败,是因为"操作者没开收集"本身不是缺陷——但上面这两条需要查清,否则这个用例等于没跑。
