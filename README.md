...
**查询延迟**(`TestT4_QueryLatency`)——2,000 篇文档、200 次查询,**冷热分开**报(2026-09 复测,宿主空载 `load average 0.36`):

| 指标 | 值(2,000 篇 / 200 次查询) | 值(8,000 篇 / 200 次查询) |
|---|---|---|
| 冷查询(重启副本后的首次,含磁盘 `Load`) | **12.07 ms** | **7.82 ms** |
| p50(热) | **6.02 ms** | **7.72 ms** |
| p95 | **7.96 ms** | 9.58 ms |
| p99 | **12.44 ms** | 10.84 ms |
| 平均 | 6.47 ms | 7.82 ms |
| 全零向量对照 p50 | 4.92 ms | 7.30 ms |

**同一台机器、同一口径下的对比**(2,000 篇,2026-09;见下"查询路径的三处修复"):
...
| 再把控制面读的连接复用起来 | 3.80 ms | 2.93 ms | 4.56 ms | 8.94 ms | 45 s |
| **本轮复测(查询向量口径修正后,见下)** | **12.07 ms** | **6.02 ms** | 7.96 ms | 12.44 ms | 45 s |

**读延迟为什么比上一轮"变大"**:这不是回归,而是**查询向量口径被修正**。上一轮的数字用 `rng.Float32()*2-1`(含负分量)生成查询向量,而 embedder 的输出是**全非负**的(`embed.deterministicVector` 就是 `hash 字节 / 255` 再归一化)。含负分量的查询向量与文档向量的相似度有正有负、区分度高,HNSW 的剪枝因此偏乐观;同卦限下相似度**全为正**,剪枝更弱、遍历更久——这才是真实调用方的处境。修正后 2,000 篇 p50 由 2.93 ms 升到 6.02 ms,而 4× 数据(2,000 → 8,000 篇)的放大只有 **1.28×**(6.02 → 7.72 ms):规模效应依然平缓,只是基数更诚实。

**冷热差距已经收窄到同一量级**(2,000 篇 cold 12.07 ms vs p50 6.02 ms;8,000 篇 cold 7.82 ms vs p50 7.72 ms)——产物 `Load` 本身是 ms 级,不再是数量级差异。
...
**查询成本的构成**(同一轮,存储节点内的分段计时 `query: stage timings`,224 个样本)——服务站看到的 6.02 ms 里,节点内占 **4.06 ms**,其余是网络与服务站的排队/路由:

| 段 | 做什么 | p50 | 平均 | p95 |
|---|---|---|---|---|
| `filter_us` | 候选的版本过滤 + 聚合(其中 `chunkmap_us` 是它的子项) | **2708 µs** | 3095 µs | 4845 µs |
| ↳ `chunkmap_us` | 每个候选 chunk 一次 `ListDocIDs`(chunk → 文档) | 1580 µs | 1769 µs | 2856 µs |
| `meta_us` | 向控制面读版本元数据(`ListVersions`,连接已复用) | 384 µs | 438 µs | 757 µs |
| `search_us` | vecstore 的 HNSW 检索 | 332 µs | 382 µs | 656 µs |
| `bloom_us` | bloom 过滤器 + 该版本的文档 ID 集合(各一次本地读) | 225 µs | 277 µs | 342 µs |
| `read_us` | 读命中文档的正文 | 138 µs | 152 µs | 241 µs |
| **`total_us`** | 节点内合计 | **4062 µs** | 4584 µs | 6669 µs |

读法:各段 p50 之和看起来大于 `total_us`,因为 `chunkmap_us` 是 `filter_us` 的子项(它在过滤循环内部累加)。真正的大头是**与候选规模成正比**的那两项(`chunkmap_us` + 过滤本身),而不是检索——`search_us` 只占 8%。这也说明这条路径对 top-k 与 `candidate_n` 敏感、而对文档总量不敏感。

> 分段计时在 **debug 级别**下取得:日志本身会给延迟加一点开销,所以**绝对值以 info 级别的冷热分位为准**(上表),这两张分步表的用途是**看构成与占比**。

**查询向量的口径**:套件发**非零、确定性、且与 embedder 同卦限**的向量(`queryVector(768)`,固定种子,分量 `rng.Float32()`)。曾经每个查询都发全零向量 —— 它与所有文档等距,HNSW 贪心遍历无从剪枝、退化为近似全图扫描;那只是**放大器**:延迟掉到 ms 级后,保留的零向量对照是 **4.92 ms vs 6.02 ms**(2,000 篇)与 **7.30 ms vs 7.72 ms**(8,000 篇),而不是数量级。

**查询路径的三处修复(2026-09)**:前两处在 `service/query.go`,共同点是把"与命中规模成正比"的工作变成"与需要的结果成正比";第三处在控制面客户端,是把"每次查询重来一遍的连接开销"变成一次性成本。
...
3. **每次查询都新建一条 gRPC 连接去控制面读元数据** —— 存储节点的每次查询都要向控制面要一次版本元数据(`ListVersions`),而 `RemoteRaftNode` 当时是**每次调用 `grpc.NewClient`、用完立刻 `Close`**:为一份控制节点从内存 map 里直接给出的数据,付一次完整的 TCP + HTTP/2 建连/断连。实测这一步占单次查询 **72–95%**(`meta_us: 6758–22337`,而检索+过滤+读正文合计只有 ~1–2.5 ms)。改成按控制地址复用一条长连接后,`meta_us` 落到 **233–463 µs**,节点内 `total_us` 从 9.3–23 ms 降到 **1.2–2.1 ms**。
...
| 指标 | 值 |
|---|---|
| 写入(`CreateVersion` × 10 批,仅 API 用时) | **372.5 ms** |
| 累计索引构建(10 批串行,每批等前一批 READY) | **2 m 16 s** |
| 端到端 | **2 m 16.6 s** |
| 存储占用(3 个存储节点合计) | **1153.1 MiB** |
| 查询 | top-k=10 返回 10 条结果 |

读法:**写入 API 从 2.55 s 降到 372 ms(约 7×)**,与控制面连接复用是同一处收益(写入路径的 `proposeAt` 也复用连接了)。索引构建由上一轮的 4 m 7 s 降到 **2 m 16 s**,这部分来自**写入侧 step 3 的并发化**:1000 篇一批的本地事务(切分 → embed → 落盘)原本逐篇串行,同口径实测 14.6 s、其中 10 ms/篇是 embedder 往返;改为 8 路有界并发后,1000 篇的 `build-accum` 由 19.6 s 降到 5.5 s。端到端仍由索引构建主导 —— "每版本独立索引"模型下吞吐由构建决定。存储占用是**节点数据目录总量**(含卷里累积的历史知识库),跨轮次不可直接比较。

**写入成本的构成**(存储节点内的分段计时 `write: stage timings`;单篇那列取自 `TestTwoTier` 的同口径样本):

| 段 | 做什么 | 单篇(`changes=1`) | **1,000 篇(并发化后)** | 1,000 篇(并发化前) |
|---|---|---|---|---|
| `acquire_us` | 该知识库在途写入的限流(§7.7) | 1 µs | 2 µs | 1 µs |
| `local_us` | 本地事务:切分 → embed → 落盘 | 16.3 ms | **2.54 s** | **15.07 s** |
| `fanout_us` | 复制到副本(quorum;对端各自拉取并落盘) | 2.6–4.3 ms | **3.87 s** | 4.55 s |
| `report_us` | 上报"数据可持久" + 触发索引构建 | 2.0–7.8 ms | **4.6 ms** | 3.2 ms |
| `confirm_us` | 广播确认(异步触发,基本不计入) | ~0 | 4 µs | 2 µs |
| **`total_us`** | 节点内合计 | **22.7–26.7 ms** | **6.41 s** | 19.63 s |

读法:**并发化把 `local_us` 从 15.07 s 压到 2.54 s(5.9×)**,于是这一批的瓶颈第一次从"本节点串行处理 1,000 篇"变成"副本也要各自落盘 1,000 篇"——`fanout_us` 3.87 s 与对端的 `local_us` 同量级,因为推过去的只是版本身份,真正的写入发生在对端(§7.5)。`report_us` 很小,却是最不能少的一步:它同时是"数据已可持久"的上报与索引构建的触发点,被整条跳过正是此前"版本永远停在 PENDING"的成因。

```bash
STRATUM_VOLUME_DOCS=10000 go test ./integration/docker/ -tags=docker -count=1 -run TestT4_DataVolume -v
```

**写入路径的修复(2026-09)**:这批压测把写入侧的账也拆开之后,下面四处各自独立的问题都露出了各自的数字:

1. **`fanOut` 里每个副本继承整个候选预算** —— 每个 push 都拿到候选的 dispatch 预算(地板 15 s + 50 ms/文档),而 fan-out 要等**全部**目标,于是一个不可达副本独吞整个预算:实测 `fanout_us = 15026509`(恰好等于 15 s 预算),一次**已经达到 quorum** 的写入被当成候选失败。现在每个 push 有自己的 3 s 预算,并且 **quorum 一达成就返回**(慢副本转后台自行结束)。
2. **失败即中断,上报被整条跳过** —— `local` 事务已经提交(文档、docID 集、WAL commit 记录都在本节点),但 `fanOut` 一失败就 `return`,`reportAndSchedule` 于是从不执行:1,000 篇一批在三个副本上的 `report_us` **全是 0**,版本停在 PENDING、查询一律 `index not ready`,数据躺在盘上没人能建索引也没人能服务它。现在把"调度构建"与"上报可持久"拆开:失败路径也调度构建,而"可持久"仍然只在 quorum 达成时才上报——那是 quorum 才能做的声明,没有 quorum 就上报等于撒谎。push 预算同时随批量缩放(3 s + 20 ms/篇)。
3. **两处"只拒绝、不修复"的死锁** —— (a) PENDING 的版本被查询直接拒绝,而 §8.6b 的惰性构建恰恰靠"有人查询"来触发;(b) 落后的副本被新鲜度检查直接拒绝,而这条路径上没有任何东西会去拉取它缺失的数据(写入到达 quorum 就够了,通知它去拉取的广播又是 best-effort)。两处都改成"**先请求补齐/构建,再拒绝**",且拒绝带可重试的 wire name(`index_not_ready` / `version_pending`)——否则服务站会把业务性拒绝当成终态:既不换副本,又把健康节点记进熔断器,实测三个存储节点全被熔断、180 s 窗口全耗在这上面。触发的补齐还必须跑在**请求之外**:挂在调用方的 ctx 上会被它自己的 deadline 掐断(实测每次都是 `DeadlineExceeded`,永远补不上),失败后要带退避重试。
4. **写入 step 3 逐篇串行** —— 见上表:`local_us` **15.07 s → 2.54 s**(5.9×)。

修复前 `TestTwoTier_EveryStorageNodeServesTheWrittenVersion` 稳定 180 s 超时、`TestT4_DataVolume` 稳定 600 s 超时;修复后分别是 **3.3 s** 与 **6.6 s**(1,000 篇)。
...
| 用例 | 测什么 | 实测 |
|---|---|---|
| `TestT4_QueryLatency` | 单版本查询延迟,**冷热分开报** | 2,000 篇 / 200 次查询:cold **12.07 ms** · p50 **6.02 ms** · p95 **7.96 ms** · p99 **12.44 ms**;8,000 篇:cold **7.82 ms** · p50 **7.72 ms**(与 embedder 同卦限的确定性非零查询向量;同口径最初为 cold 769 ms / p50 679 ms) |
| `TestT4_DataVolume` | 写入量:写 / 构建 / 占用 | 10,000 篇(10 批 × 1,000):写入 API **372.5 ms** · 累计索引构建 **2 m 16 s** · 三节点占用 **1153.1 MiB** |
| `TestTwoTier_EveryStorageNodeServesTheWrittenVersion` | 控制组提交的版本在每个存储副本上都能读到 | 每个存储副本都能服务写过的版本(**3.3 s**) |
